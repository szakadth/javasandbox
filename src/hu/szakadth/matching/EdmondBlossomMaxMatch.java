...
public class EdmondBlossomMaxMatch {
    ...
    int readTotalGraphCount(BufferedReader bufReader) throws Exception {
        return Integer.parseInt(getNextLineSkipComment(bufReader));
    }

    private String getNextLineSkipComment(BufferedReader bufReader) throws Exception  {
    ...
    void readNextGraph(BufferedReader bufReader) throws Exception {
        try {
            ...
            int nodesCount = Integer.parseInt(getNextLineSkipComment(bufReader));
            int edgesCount = Integer.parseInt(getNextLineSkipComment(bufReader));
            g = new Graph();
            g.nodeCount = nodesCount;
            initGraph();
            for (int k = 0; k < edgesCount; k++) {
                String[] strArr = getNextLineSkipComment(bufReader).split(" ");
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            ...
            try {
                bufReader = new BufferedReader(new FileReader(".\\maxmatch.txt"));
            } catch (Exception e) {
                e.printStackTrace();
                return;
            }
